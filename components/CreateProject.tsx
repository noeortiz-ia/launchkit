...
const CreateProject: React.FC = () => {
  ...
  const handleExtract = async () => {
    if (!apiKey) {
        alert(t("Por favor, configura tu API Key de OpenRouter en la configuración (⚙️) antes de procesar.", "Please configure your OpenRouter API Key in settings (⚙️) before processing."));
        openSettings();
        return;
    }
    ...
    setLoading(true);
    try {
      const extracted = await extractPRD(prdText, { apiKey, textModel, imageModel }, language);
      setFormData(prev => ({ ...prev, ...extracted }));
      setMode('manual'); // Switch to manual review
    } catch (e) {
      console.error(e);
      alert(t('Error al extraer información. Por favor intenta de nuevo.', 'Error extracting information. Please try again.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    if (!formData.name || !formData.description) {
        alert(t("Por favor completa al menos el nombre y la descripción", "Please complete at least the name and description"));
        return;
    }
    ...
    try {
        ...
        alert(t("Error al crear el proyecto. Intenta nuevamente.", "Error creating project. Try again."));
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10 animate-fadeIn">
      ...
      >
        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> {t('Volver', 'Back')}
      </button>
      
      <h1 className="text-3xl font-bold text-textMain mb-2">{t('Nuevo Proyecto', 'New Project')}</h1>
      <p className="text-textSec mb-8">{t('Ingresa la información de tu producto para que LaunchKit genere tu plan.', 'Enter your product information for LaunchKit to generate your plan.')}</p>
      ...
      <div className="flex border-b border-border mb-8">
        <button
          ...
          onClick={() => setMode('ai')}
        >
          <Sparkles className="w-4 h-4" /> {t('Extraer info con IA', 'Extract info with AI')}
        </button>
        <button
        ...
        >
          <FileText className="w-4 h-4" /> {t('Manual', 'Manual')}
        </button>
      </div>
      ...
      {mode === 'ai' && (
        <div className="animate-fadeIn">
          <label className="block text-sm font-medium text-textSec mb-2">
            {t('Pega tu PRD, descripción de App Store o documento de producto', 'Paste your PRD, App Store description, or product document')}
          </label>
          <textarea
            ...
            placeholder={t("Ej: LaunchKit es una herramienta que ayuda a creadores a...", "E.g.: LaunchKit is a tool that helps creators to...")}
            value={prdText}
          ...
          >
            {loading ? (
                ...
                t('Extraer Información', 'Extract Information')
            )}
          </button>
        </div>
      )}
      ...
      {mode === 'manual' && (
        <div className="space-y-6 animate-fadeIn">
          <div>
            <label className="block text-sm text-textSec mb-1">{t('Nombre del Producto / App', 'Product / App Name')}</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm text-textSec mb-1">{t('Descripción (¿Qué hace?)', 'Description (What does it do?)')}</label>
            <textarea
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label className="block text-sm text-textSec mb-1">{t('Público Objetivo', 'Target Audience')}</label>
                <textarea
            ...
            <div>
                <label className="block text-sm text-textSec mb-1">{t('Problema que Resuelve', 'Problem Solved')}</label>
                <textarea
                className="w-full bg-surface border border-border rounded p-3 text-textMain focus:border-accent focus:outline-none h-40 resize-none"
